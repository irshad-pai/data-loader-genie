...
class NifiPopulator():
    ...
    def populate(self, execution_config):
        if "s3" in execution_config.get("source").keys():
            ...
            data_source_processor_groups = nipyapi.nifi.apis.ProcessGroupsApi().get_process_groups(
            ...
            if execution_config.get("source").get("name") not in data_source_processor:
                ad_receiver_pg = nipyapi.canvas.create_process_group(s3_process_group,
                                                                     ...
                                                                     comment=f"{execution_config.get('source').get('name')} S3 process group")
                ...
                if (bool(execution_config.get("source").get("isMultiline"))):
                    multiline = "multiline"
                else:
                ...
                nipyapi.canvas.create_connection(list_s3_processor, fetch_s3_processor, relationships=['success'],
                                                 name=None)
                nipyapi.canvas.create_connection(fetch_s3_processor, ad_update_attr_processor,
